...
local launcher_interrupt = { ignore_mods = true, non_consuming = true }
...
-- Super tap opens Caelestia search. 0.56 does not deliver GlobalShortcut
-- on modifier-release, so this goes through shell IPC instead.
create_bind(vars.kbLauncher, hl.dsp.exec_cmd("caelestia shell drawers toggle launcher"), release)

for _, btn in ipairs({ 272, 273, 274, 275, 276, 277 }) do
    hl.bind("SUPER + mouse:" .. btn, hl.dsp.global("caelestia:launcherInterrupt"), launcher_interrupt)
end
hl.bind("SUPER + mouse_up", hl.dsp.global("caelestia:launcherInterrupt"), launcher_interrupt)
hl.bind("SUPER + mouse_down", hl.dsp.global("caelestia:launcherInterrupt"), launcher_interrupt)

create_bind(vars.kbSession, hl.dsp.global("caelestia:session"))
create_bind(vars.kbShowSidebar, hl.dsp.global("caelestia:sidebar"))
create_bind(vars.kbClearNotifs, hl.dsp.exec_cmd("caelestia shell notifs clear"), locked)
...
create_bind(vars.kbTerminal, hl.dsp.exec_cmd(fn.app_cmd(vars.terminal)))
create_bind(vars.kbBrowser, hl.dsp.exec_cmd(fn.app_cmd(vars.browser)))
create_bind(vars.kbEditor, hl.dsp.exec_cmd(fn.app_cmd(vars.editor)))
create_bind(vars.kbFileExplorer, hl.dsp.exec_cmd(fn.app_cmd(vars.fileExplorer)))
create_bind(vars.kbAudioSettings, hl.dsp.exec_cmd(fn.app_cmd(vars.audioSettings)))
create_bind(vars.kbTelegram, hl.dsp.exec_cmd(fn.app_cmd(vars.messenger)))
create_bind(vars.kbGithub, hl.dsp.exec_cmd(fn.app_cmd(vars.gitClient)))
create_bind(vars.kbProcessViewer, hl.dsp.exec_cmd(fn.app_cmd(vars.processViewer)))

create_bind(vars.kbScreenshot, hl.dsp.exec_cmd("caelestia screenshot"), locked)
create_bind(vars.kbScreenshotClipboard, hl.dsp.global("caelestia:screenshotFreezeClip"), locked)
create_bind(
    vars.kbScreenshotRegion,
    hl.dsp.exec_cmd(os.getenv("HOME") .. "/.config/hypr/scripts/screenshot-region.sh")
)
