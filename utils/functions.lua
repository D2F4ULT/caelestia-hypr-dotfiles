local function wsaction(action, range, i)
    return function()
        ...
        local grouped = (i - 1) * 10 + (id % 10)
        local in_group = math.floor((id - 1) / 10) * 10 + i
        local target = (range == "group") and grouped or in_group
...
local function move_actions(win)
    local screen = hl.get_active_monitor()
    if not (screen and screen.width and screen.height and win and win.size) then
        return
    end
...
local function default_config()
    return {
        communication = {
            equibop = {
                enable = true,
                match = { { class = "equibop" }, { class = "discord" }, { class = "vesktop" } },
                command = { "app2unit", "--", "equibop" },
                move = true,
            },
...
local function load_toggle_config()
    ...
    if recognized and type(conf_or_error) == "table" then
        ...
        hl.exec_cmd("caelestia shell toaster error " .. shell_join({ "Failed to parse CLI config", reason }) .. " error")
    end
...
local function app_cmd(name)
    local vars = require("variables")
    return (vars.appLauncher or "") .. name
end
